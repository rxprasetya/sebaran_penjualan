...
const SalesCoverageArea = () => {
    ...
    const onDelete = async (id: number) => {
        ...
        if (result.isConfirmed) {
            try {
                await fetch(`http://localhost:3000/api/salescoverageareas/delete/${id}`, {
                    method: 'DELETE'
                });
                            ...
                            <tbody>
                                {salesCoverageAreas.map((item: any, idx: number) => (
                                    <tr key={idx}>
                                        ...
                                        <td>
                                            <div className="flex">
                                                <Link className='badge bg-success me-2' to={`/sales-coverage-area/update/${item.id}`}>Update</Link>
