...
const SalesCoverageAreaActionForm = () => {
    ...
    const init = async () => {
        if (id) {
            const res = await fetch('http://localhost:3000/api/salescoverageareas/search/' + id, { method: 'GET' });
    ...
    const fetchSales = async () => {
        try {
            const res = await fetch('http://localhost:3000/api/salescoverageareas/sales', { method: 'GET' })
    ...
    const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            const res = await fetch(isUpdate
                ? 'http://localhost:3000/api/salescoverageareas/update/' + id
                : 'http://localhost:3000/api/salescoverageareas/create', {
            ...
            <div className="page-title">
                <div className="row">
                    <div className="col-12 col-md-6 order-md-1 order-last">
                        <h3>{isUpdate ? 'Update Sales Area' : 'Create Sales Area'}</h3>
                    ...
                    <div className="col-12 col-md-6 order-md-2 order-first">
                        <nav aria-label="breadcrumb" className="breadcrumb-header float-start float-lg-end">
                            <ol className="breadcrumb">
                                ...
                                <li className="breadcrumb-item">
                                    <Link to="/sales-coverage-area">Datatable Sales Area</Link>
                                </li>
                                <li className="breadcrumb-item active" aria-current="page">{isUpdate ? 'Update Sales Area' : 'Create Sales Area'}</li>
                            </ol>
                        </nav>
                    </div>
                </div>
                                            ...
                                            <div className="form-group">
                                                ...
                                                <select id="employeeID" className="form-control" value={employeeID} onChange={(e) => setEmployeeID(e.target.value)}>
                                                    ...
                                                    {sales.map((item: any, idx: number) => (
                                                        <option key={idx} value={item.id}>[ {item.code} ] {item.name}</option>
                                                    ))}
                                                </select>
...
export default SalesCoverageAreaActionForm